...
function TopRated() {
  ...
  useEffect(() => {
    async function fetchRes() {
      ...
      if (pathname === "/movies") {
        ...
        const result = await fetch(
          `https://api.themoviedb.org/3/tv/top_rated?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US&page=${page}`
        ).then((res) => res.json());
        setPlaying(result.results);
      }
    }
  ...
  return (
    <div className="text-white px-10">
      <div className="flex">
        <h4 className="text-xl mb-3">Top Rated</h4>
      ...
      <Swiper
        modules={[Navigation]}
        navigation={{ nextEl: ".ascend", prevEl: ".descend" }}
        slidesPerView="auto"
        className="!flex !relative"
      >
        <div className="absolute flex justify-center items-center m-auto h-full top-0 right-0 left-0 bottom-0">
          <ChevronLeftIcon className="w-10 h-10 z-50 cursor-pointer descend" />
          <ChevronRightIcon className="w-10 h-10 ml-auto z-50 cursor-pointer ascend" />
        </div>
